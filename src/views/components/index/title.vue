...
<template>
  <div class="size-full flex justify-between items-center px-[20px]">
    ...
    <div class="h-full flex items-center gap-[10px]">
      ...
      <div class="text-[18px] font-bold">
        Ximo Muisc <span class="text-primary">Player</span>
      </div>
      <!-- 首页按钮 -->
      <Button
        variant="outline"
        size="sm"
        class="ml-[20px] h-[65%] text-[13px] hover:text-primary"
        @click="goRouter('/')"
      >
        首页
      </Button>
      ...
      <!-- 项目仓库按钮 -->
      <a
        href="https://github.com/Xxmm153/cloudMusic_ximo"
        class="h-[65%]"
        target="_blank"
      >
        <Button
          variant="outline"
          size="sm"
          class="ml-[20px] gap-[5px] h-full text-[13px] hover:text-primary"
        >
          <Github class="size-[12px]" />项目仓库<ArrowUpRight
            class="size-[13px]"
          />
        </Button>
      </a>
    ...
    <div class="flex gap-[10px] h-full items-center">
      ...
      <Button
        ...
        class="h-[65%] hover:text-primary"
      >
        <MoonStar v-if="!isDrak" class="size-[13px]" />
        <Sun v-else class="size-[13px]" /><span class="text-[13px]">
          {{ !isDrak ? "黑夜" : "白天" }}</span
        >
      </Button>
      <Button
        @click="showThemDialog"
        variant="outline"
        size="sm"
        class="h-[65%] hover:text-primary"
        ><Sparkles class="size-[13px] text-primary" /><span class="text-[13px]"
          >切换主题</span
        ></Button
      >
      <!-- 登录按钮 -->
      ...
      <Button
        ...
        size="sm"
        class="h-[65%] hover:text-primary"
        ><User class="size-[13px]" /><span class="text-[13px]"
          >登录</span
        ></Button
      >
    </div>
  ...
  <Dialog v-model:open="loginDialogShow" @update:open="closeDialog">
    <form>
      <DialogContent
        class="sm:max-w-1/4 transition-all duration-300"
        @click="DialogContentClick"
      >
        <DialogHeader>
          <DialogTitle class="text-2xl">登录Login</DialogTitle>
          <DialogDescription>
            当前登录方式为<span class="text-primary">{{ loginType.text }}</span
            >,当然你也可以选择剩下两种登录方式(
            <span
              v-for="(i, index) in loginTypeOther"
              :key="i.value"
              class="text-primary"
              >{{ i.text }} <span v-if="!index">、</span></span
            >
            )
          </DialogDescription>
        </DialogHeader>
        <div
          class="h-55 flex flex-col justify-center items-center"
          v-if="loginType.value === 'qr'"
        >
          <div class="size-38 relative">
            ...
            <div
              v-show="!qrUrl"
              class="flex justify-center items-center size-38"
            >
              <div class="relative w-24 h-24">
                ...
                <!-- 内圈反向旋转 -->
                <div
                  class="absolute inset-2 rounded-full border-2 border-primary/10 border-b-primary animate-spin-reverse"
                ></div>
                <!-- 中心花朵 -->
                <div class="absolute inset-4 flex justify-center items-center">
                  <div
                    class="w-8 h-8 rounded-full bg-primary/30 animate-pulse"
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div
              v-show="avatarUrl"
              class="absolute size-full z-50 bg-black/80 flex flex-col justify-center items-center"
            >
              ...
              />
              <div
                v-show="!noUseQr && nickname"
                class="text-sm mt-1 text-primary font-bold"
              >
                {{ nickname }}
              </div>
            </div>
          ...
          <div class="text-sm mt-1">
            {{ qrMessage ? qrMessage : "请使用网易云app扫码" }}
          </div>
        ...
        <div class="grid gap-4" v-else>
          <div class="flex flex-col gap-1">
            ...
            <Input
              v-model="sendLoginData.user"
              id="name-1"
              name="name"
              :placeholder="
                loginType.value === 'phone' ? '手机号 这儿~' : '邮箱 这儿~'
              "
              class="rounded-sm !h-8"
              :class="{ 'border-primary': uerWarning }"
            />
            <div v-show="uerWarning" class="text-[11px] ml-1 text-primary">
              {{
          ...
          <div class="flex flex-col gap-1">
            ...
            <Input
              type="password"
              v-model="sendLoginData.password"
              id="username-1"
              name="username"
              placeholder="密码 这儿~"
              class="rounded-sm !h-8"
              :class="{ 'border-primary': passwordWarning }"
            />
            <div v-show="passwordWarning" class="text-[11px] ml-1 text-primary">
              {{ "密码不能为空哦~" }}
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          <div class="flex-1 flex items-end gap-4">
            <span
              v-for="i in loginTypeOther"
              @click="changeLogin(i)"
              class="text-[13px] items-end hover:underline cursor-pointer transition-all"
            >
              {{ i.text }}
            </span>
    ...
    <DialogContent class="sm:max-w-1/4 transition-all duration-300">
      ...
      <div class="thembox py-4 px-5 m-h-1/3">
        ...
        <div class="thembox-t h-fit font-bold">
          ...
          <div
            class="t-t-r iconfont icon-zhuti_tiaosepan !text-2xl text-primary"
          ></div>
        </div>
        <!-- <div class=" mt-0">
        Cycle between 9 themes and choose your favorite theme to apply~.
      </div> -->
        <div class="mt-0">在9个主题之间循环，选择你最喜欢的主题来使用吧~。</div>
        <!-- 这里主题可以写数组进行循环 当时主题少所以就一个个加上去了  随后变多了hhh  你可以优化成数组改一下 -->
        <div class="them-con my-6">
          <!-- Rose Theme -->
          <div
            class="them-con-item rose bg-primary"
            :class="{ 'them-ac': currentThem === 'rose' }"
            @click="changeThem('rose')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- Purple Theme -->
          <div
            class="them-con-item Purple bg-primary"
            :class="{ 'them-ac': currentThem === 'Purple' }"
            @click="changeThem('Purple')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- Green Theme -->
          <div
            class="them-con-item green bg-primary"
            :class="{ 'them-ac': currentThem === 'green' }"
            @click="changeThem('green')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- Dark Theme -->
          <div
            class="them-con-item dark bg-background"
            :class="{ 'them-ac': currentThem === 'dark' }"
            @click="changeThem('dark')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- Red Theme -->
          <div
            class="them-con-item red bg-primary"
            :class="{ 'them-ac': currentThem === 'red' }"
            @click="changeThem('red')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- Orange Theme -->
          <div
            class="them-con-item orange bg-primary"
            :class="{ 'them-ac': currentThem === 'orange' }"
            @click="changeThem('orange')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- Blue Theme -->
          <div
            class="them-con-item blue bg-primary"
            :class="{ 'them-ac': currentThem === 'blue' }"
            @click="changeThem('blue')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- Yellow Theme -->
          <div
            class="them-con-item yellow bg-primary"
            :class="{ 'them-ac': currentThem === 'yellow' }"
            @click="changeThem('yellow')"
          >
            <div class="top">
              <div class="h-5"></div>
            </div>
          ...
          <!-- DeepPink Theme -->
          <div
            class="them-con-item deeppink bg-primary"
            :class="{ 'them-ac': currentThem === 'deeppink' }"
            @click="changeThem('deeppink')"
          >
            <!-- <div class="top">
        ...
        <div class="bu">
          <Button variant="outline" @click="themShowDialogClose"> 不要 </Button>
          <Button @click="themShowDialogOk"> 就要就要 </Button>
        </div>
      </div>
    </DialogContent>
  </Dialog>
...
import {
  ...
  MoonStar,
  Sun,
} from "lucide-vue-next"; //使用lucide图标
...
const drakMode = () => {
  ...
  localStorage.setItem(
    "themeXimo",
    localStorage.getItem("themeOther") || "rose",
  );
